...
class LibriSpeechDataset(Dataset):
    ...
    def __init__(self, 
        ...
        self.bad_audio_files = set()

        if bad_audio_file is not None and os.path.exists(bad_audio_file):
            with open(bad_audio_file, "r") as bad_file:
                for line in bad_file:
                    file_path = line.split("\t")[0].strip()
                    if file_path:
                        self.bad_audio_files.add(os.path.abspath(file_path))
        elif bad_audio_file is not None and not os.path.exists(bad_audio_file):
            print(f"[Wav2Vec2 dataset] Warning: bad audio file list '{bad_audio_file}' does not exist. Continuing without it.")
            ...
            for speaker in os.listdir(path_to_split):
                ...
                for section in os.listdir(path_to_speaker):
                    ...
                    for line in transcripts:
                        ...
                        if os.path.abspath(full_path_to_audio_file) in self.bad_audio_files:
                            continue
    ...
    def __getitem__(self, idx):
        ...
        except Exception as partial_read_error:
            try:
                audio, orig_sr = torchaudio.load(path_to_audio)
                audio = audio[..., :self.max_audio_samples]
            except Exception as full_read_error:
                raise RuntimeError(
                    f"Failed to load audio file: {path_to_audio}. "
                    f"Partial read error: {partial_read_error!r}. "
                    f"Full read error: {full_read_error!r}."
                ) from full_read_error
