...
class TTSDataset(Dataset):
    def __init__(self, 
        ...
        self.path_to_mels = path_to_mels
    ...
    def __getitem__(self, idx):
        ...
        if "mel_path" in sample and isinstance(sample["mel_path"], str) and sample["mel_path"]:
            mel = torch.load(sample["mel_path"], map_location="cpu").float()
        elif self.path_to_mels is not None:
            file_stem = os.path.splitext(os.path.basename(path_to_audio))[0]
            mel = torch.load(os.path.join(self.path_to_mels, f"{file_stem}.pt"), map_location="cpu").float()
        else:
            audio = load_wav(path_to_audio, sr=self.sample_rate)
            mel = self.audio_proc.audio2mel(audio, do_norm=True).squeeze(0)
